...
impl NoSuchChunk {
    pub fn new() -> NoSuchChunk {
        NoSuchChunk {}
    }
}

impl Display for NoSuchChunk {
...
impl Png {
    ...
    fn chunk_by_type(&self, chunk_type: &str) -> Option<Chunk> {
        ChunkType::from_str(chunk_type)
            ...
            .and_then(|to_find| {
                self.chunks.iter()
                    .find(|&chunk| *chunk.chunk_type() == to_find)
                    .cloned()
            })
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        if header_buffer != Png::STANDARD_HEADER {
            return Err(Box::new(InvalidHeader::new()));
        }
        ...
        loop {
            ...
            let mut rest = vec![0; 8+length as usize];
            reader.read_exact(rest.as_mut_slice())?;

            let chunk: Chunk = buffer.iter()
                .chain(rest.iter())
                .cloned()
                .collect::<Vec<u8>>()
                .as_slice()
